...
class TestJWTAuthentication:
    ...
    def create_user_and_token(self, token_type='access', is_active=True):
        user = (
            ...
            if is_active else
            InactiveUserFactory(phone=self.phone, password=self.password)
        )
        payload = {
            'id': user.id,
            'phone': user.phone,
    ...
    def test_invalid_header_format(self):
        request = self.factory.get(self.url, HTTP_AUTHORIZATION="BearerTokenWithoutSpace")
    ...
    def test_invalid_token(self):
        request = self.factory.get(self.url, HTTP_AUTHORIZATION="Bearer invalid.token.here")
